...
*Tracked live, judged live. The mesh is MediaPipe's; the instruction, the
checklist and the priority chain are this project's `pipeline/guidance.py` —
fourteen rules in a fixed order, of which only the first to fire is shown, so a
person gets one thing to do instead of a list. `tools/build_static.py` refuses
to publish the JavaScript if it disagrees with the Python.*

> **The recording** was made by handing a still photograph to the browser as if
> it were a camera, and the frame is painted over *after* the measurements are
> read — so the GIF carries the tracking and nobody's face. Open the page and it
> is your own camera; no frame leaves the tab.
...
machine it is plugged into — which is why the demos above cover only the parts
that are pure arithmetic. **Don't skip the middle line:** attendance works
without those weights, but traits, liveness, quality scoring and SFace
identification degrade quietly, which looks like the app being broken rather
than incomplete. `GET /api/models` names the missing ones.
...
does the same — `cli.register_user`, `cli.attendance`, `cli.view_report` — and
`cli.seed_demo --people 40 --samples 10` enrolls from LFW to try recognition
without registering anyone. Every feature, with the endpoint and command behind
it, is [listed on the camera page](https://luke-zhang-cs-py.github.io/Facial-Recognition-Software/camera/#features).
...
- **The right threshold depends on how many people are enrolled** — one swept
  ...
  That is the birthday problem, and why `analysis/calibration.py` takes a
  gallery size instead of returning a constant.
...
- **Nothing gates on skin tone.** Exposure advice fires on clipped pixels, not
  ...
  and dark lenses genuinely occlude, so those are named, and only when detection
  is actually failing.
- **Age** is one figure with a ±6-year band, shown *with how often that band is
  right, which is ~40%*. That pairing is the point. MAE 12.4 years over 10,946
  faces. **Gender** guesses apparent presentation from pixels and says nothing
  about anyone's identity. Both softmax over a fixed label set, so they return a
  confident label for *anything*, including a black frame; the readout
  suppresses them when no face is detected.
