...
across the tools, and three of them were wrong:
  ...
  - tools/trials.py read os.environ["TEMP"], which does not exist outside
  ...
  - tools/benchmark_loop.py had FairFace under a Claude Code session
...
def sample_frame_dir():
    ...
    permanently. `python tools/sample_frames.py <video-or-folder>` fills it.
    """
