...
attendance.py` working on a machine that never ran `cli/fetch_models.py`.
...
def models_dir():
    """Where the weights live, asked for per call rather than held.

    This used to be `MODELS_DIR = os.path.join(os.path.dirname(__file__),
    "models")` -- a second opinion about a directory `paths.py` already owns,
    and one that would have gone on resolving perfectly happily to
    `core/models/` once this file moved into a package. Nothing would have
    raised; `available()` would simply have reported every model missing.
    """
...
def missing_summary():
    ...
    if not gone:
        return None
    return f"{len(gone)} model(s) not downloaded: {', '.join(gone)}. Run: python -m cli.fetch_models"
