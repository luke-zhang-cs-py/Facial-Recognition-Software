...
def model_path():
    """The 68-point LBF model, asked for per call.

    This was `os.path.join(os.path.dirname(__file__), "models", ...)`, which
    is a second opinion about a directory `paths.py` already owns -- and one
    that would have quietly started naming `pipeline/models/lbfmodel.yaml`
    when this file moved into a package. `available()` would then have said
    the model was absent and the mesh would simply have stopped being drawn,
    with nothing raising anywhere.
    """
    return os.path.join(paths.models_dir(), "lbfmodel.yaml")
