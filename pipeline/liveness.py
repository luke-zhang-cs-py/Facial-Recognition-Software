...
def model_path():
    """The anti-spoof net, asked for per call rather than held.

    Same reason as landmarks.model_path(): built from this file's own
    __file__ it pointed at `pipeline/models/` after the move, `available()`
    would have returned False, and the only symptom would have been that
    photographs stopped being refused.
    """
    return os.path.join(paths.models_dir(), "minifasnet_v2.onnx")
