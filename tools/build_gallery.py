...
thousand people is ~16 MB rather than gigabytes of face crops nobody needs.
    ...
    python tools/build_gallery.py [--shards N] [--per-id 4] [--workers 8]

Writes where tools/scale_benchmark.py reads, via corpus_paths -- set
...
# The project root -- one level up, because this file lives in tools/. The
# worker below chdir()s to it, so getting this wrong moves every relative
# path in the pool rather than raising.
PROJ = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, PROJ)

from core.corpus_paths import gallery_dir, gallery_path   # noqa: E402
